    ...
    <div className="max-w-xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">{partner.full_name}</h1>
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-3">
          <div className="border border-border rounded-xl p-4">
            ...
            <p className="text-xs text-muted-foreground/80">{stats.converted_leads} convertidos</p>
          </div>
          <div className="border border-border rounded-xl p-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-1">
              <Building2 size={14} /><span className="text-xs">Imóveis</span>
            </div>
            <p className="text-2xl font-bold text-foreground">{stats.total_properties}</p>
            <p className="text-xs text-muted-foreground/80">{stats.operational_properties} em operação</p>
          </div>
          <div className="border border-border rounded-xl p-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-1">
              <DollarSign size={14} /><span className="text-xs">A receber</span>
            </div>
            <p className="text-xl font-bold text-primary">{brl(stats.total_commissions_pending)}</p>
          </div>
          <div className="border border-border rounded-xl p-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-1">
              <TrendingUp size={14} /><span className="text-xs">Total recebido</span>
            </div>
            <p className="text-xl font-bold text-foreground">{brl(stats.total_commissions_paid)}</p>
          </div>
        </div>
      )}
      ...
      <Card className="shadow-none border-border">
        ...
        <CardContent>
          <div className="flex items-center gap-2">
            <code className="flex-1 text-xs bg-muted border border-border rounded-lg px-3 py-2 truncate text-muted-foreground">
      ...
      <Card className="shadow-none border-border">
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2">
            <Upload size={15} className="text-muted-foreground/80" />
            Documentos
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {(partner.documents ?? []).length === 0 ? (
            <p className="text-sm text-muted-foreground/80">Nenhum documento enviado.</p>
          ) : (
              ...
              {partner.documents!.map((doc) => (
                ...
                >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="shrink-0 text-muted-foreground/80 hover:text-primary transition"
                    title="Visualizar"
                  >
          ...
          <div className="space-y-3 pt-1 border-t border-border">
            ...
            <select
              ...
              className="w-full h-9 px-3 rounded-lg border border-border text-sm bg-card focus:outline-none focus:ring-2 focus:ring-primary/30"
            >
            ...
            <div
              className="border-2 border-dashed border-border rounded-xl p-4 text-center cursor-pointer hover:border-primary/50 transition"
            ...
            >
              ...
              {uploading ? (
                ...
                <>
                  <Upload size={18} className="mx-auto text-muted-foreground/60 mb-1" />
                  <p className="text-xs text-muted-foreground">
                    Arraste ou <span className="text-primary">clique para selecionar</span>
                  </p>
                  <p className="text-xs text-muted-foreground/80">PDF, JPEG ou PNG · máx. 5 MB</p>
                </>
              )}
            </div>
            {uploadError && (
